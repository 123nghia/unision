...
indexDraw ++;
let dataDrawFace  = " https://soida.placentor.com.vn/images/image1.png";
...
function openDetail(item) {
   var parrentItem = item.closest(".content-description");
   var ulShow = parrentItem.getElementsByTagName("ul")[0];
   item.classList.add('hideItem');
   setTimeout(() => {
    ulShow.classList.remove('hideItem');
   }, 500);
...
$.ajax({
 type: "POST",
 url: "https://soida-api.placentor.com.vn/itemSdk/get_product_result",
...
$.ajax({
type: "POST",
url: "https://soida-api.placentor.com.vn/api/paramenterRecomed/getAllCocludeOverView",
data: JSON.stringify(bodyRequest),
contentType: "application/json",
dataType: "json",
...
$.ajax({
type: "POST",
url: "https://soida-api.placentor.com.vn/api/paramenterRecomed/getAllCocludeDetail",
data: JSON.stringify(bodyRequest),
contentType: "application/json",
dataType: "json",
success:function(data)
...
function drawConcludeOverview2 ( groupk, valuek, item) 
{
  ...
  if(valuek >3)
    ...
    let titlegroup = "";
    let valuegroup = "Chưa có thông tin";


    if( groupk == "K5") 
    {
      titlegroup  = "Lão hoá da";
    }
    else if( groupk == "K6") 
    {
      titlegroup  = "Mụn và mụn viêm đỏ";
    }
    else if( groupk == "K7") 
    {
      titlegroup  = "Quầng thâm mắt";
    }
    else if( groupk == "K8") 
    {
      titlegroup  = "Lỗ chân lông";
    }
    else if( groupk == "K9") 
    {
      titlegroup  = "Đốm thâm nám";
    }


    for (let i = 0; i < resultKL.length; i++) {
      var item = resultKL[i];
      if (groupk ==  item.GroupProduct && item.Type == "1") 
      {
         if( valuek == item.Level)
         {
          valuegroup =   item.Content;
         }
      }
  }
...
function drawConcludev2 ( groupk, valuek, item) 
{
  ...
  var text = "Tốt";
    ...
    for (let i = 0; i < resultKL.length; i++) {
        var item = resultKL[i];
        if (groupk ==  item.GroupProduct && item.Type == "0") 
        {
           if( valuek == item.Level)
           {
            valuegroup =   item.Title;
           }
        }
    }
...
htmlTemplate+='  <div class="dataProduct">';
      ...
      dataProducts.list_product.forEach(itemProduct =>  {
         ...
         htmlTemplate += '<div class="product-item">\
                         <div> \
                         <img\
                         src="https://soida-api.placentor.com.vn/public/image_plugin/toner-Dashu-0x0.jpg"\
                         alt="">\
                         </div>\
