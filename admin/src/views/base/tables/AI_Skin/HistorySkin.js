   ...
   var indexSTT =0;
    DataExport.forEach(element => {
            ...
            var item = {
              ...
              Phone: element.Phone, 
              Image: element.Image,
              linkDetail: "https://soida-api.placentor.com.vn/xemchitietlichsu/"+ element._id,
              Slug: compnayName,
                      ...
                      data != undefined ?
                        data.map((item, i) => {
                          ...
                          if(item.Name) 
                          {
                           ...
                           let minisizeClick =  '';
                           let timetextMinisize = '';


                           if(item.connectionType =="zalo" ||  item.connectionType =="messenger")
                           {
                              connectionType=  item.connectionType;
                              timetext= (new Date(item.timeConnection)).toLocaleString('en-GB');
                           }

                           if(item.minisizeClick =="minisize" )
                           {
                              minisizeClick=  "minisize";
                              timetextMinisize= (new Date(item.timeminisizeClick)).toLocaleString('en-GB');
                           }
                          return (
...
<td className="text-left displayblock">
    ...
    { minisizeClick != "" && <><span> 
      Loại click:  <strong>{minisizeClick}</strong>
    </span>
    <span >
      Thời gian: <strong>{timetextMinisize}</strong> 
                            ...
                            {
                              saleNameText=="" && (<><span>Admin QuangAnh  </span> </>)
                            }
                              ...
                              <td className="text-center">
                                <p>{(new Date(item.Create_Date)).toLocaleDateString('en-GB')} </p> 
                                <p>{(new Date(item.Create_Date)).toLocaleTimeString('en-GB')} </p>
