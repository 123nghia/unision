...
class BrandSlider extends Component {
  ...
  getData = async () => {
    ...
    const res_brand = await axios({
      ...
      url: Constants.LIST_Banner_soida,
      method: 'GET'
    });
  ...
  getData_Company = async () => {
    ...
    const res_brand = await axios({
      baseURL: Constants.BASE_URL,
      url: Constants.LIST_Banner_soida + JSON.parse(this.state.user).company_id,
      method: 'GET'
    });
  ...
  async addBrand() {
    ...
    const { name, image, link, image_link,countSkin } = this.state
    if (name == null || name == '' ||
      image == null || image == '') {
    ...
    const res = await axios({
      baseURL: Constants.BASE_URL,
      url: Constants.ADD_Banner_soida,
      method: 'POST',
      data: body
    });
  ...
  async updateBrand() {
    ...
    const res = await axios({
      baseURL: Constants.BASE_URL,
      url: Constants.UPDATE_Banner_soida,
      method: 'POST',
      data: body
    });
  ...
  async delete() {
    ...
    const res = await axios({
      baseURL: Constants.BASE_URL,
      url: Constants.DELETE_Banner_soida,
      method: 'POST',
      data: {
        "id": this.state.id
      }
                  ...
                  <table ble className="table table-hover table-outline mb-0  d-sm-table">
                    <thead className="thead-light">
                      <tr>
                        ...
                        <th className="text-center">lượt soi</th>
                          ...
                          data.map((item, i) => {
                            return (
                              <tr key={i}>
                                ...
                                <td className="text-center">
                                  {
                                    item.image_link == null || item.image_link == "" ? <img src={`${item.image}`} width={"400px"} /> :
                                      <img src={`https://soida-api.placentor.com.vn/public/image_brand/${item.image_link}`} width={"400px"}  />
                                  }
                                </td>
                                ...
                                <td className="text-center">{item.countSkin}</td>
                                <td className="text-center">
          ...
          <Modal isOpen={this.state.modalCom} className={this.props.className}>
            ...
            <ModalBody> 
              ...
              <TextFieldGroup
                field="countSkin"
                label="Lượt soi"
                value={this.state.countSkin}
                placeholder={"Lượt soi"}
                // error={errors.title}
                onChange={e => this.onChange("countSkin", e.target.value)}
              // rows="5"
              />
              ...
              />
              {
                  ...
                  "" :
                    ...
                    this.state.image_show == "" ? `https://soida-api.placentor.com.vn/public/image_brand/${this.state.image_link}` : this.state.image} style={{ marginBottom: 20 }} />
              }
