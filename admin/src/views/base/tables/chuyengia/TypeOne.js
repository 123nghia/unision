...
class SuggestItem extends Component {
  ...
  onSearch = async () => {
    ...
    if (type == '0' || type == '1') {
      ...
      const res_suggest = await API_CONNECT(
        //https://soida-api.placentor.com.vn/itemSdk/search/611f1461ef623903dbb4f75d/1?&key=c
  ...
  getDataForCompany = async () => {
    ...
    const res_suggest = await axios({
      baseURL: Constants.BASE_URL,
      url: Constants.LIST_chuyengia_ITEM_COMPANY + JSON.parse(userData).company_id + `/${idSDK}`,
      method: 'GET'
    });
    ...
    this.setState({ dataApi: val, data: val,
        isLoading: false
  });
  ...
  async addProduct() {
    ...
    if (name == null || name == '') {
      alert("Thiếu tên chuyên gia");
      return
    }else if (image == null || image == '') {
      alert("Thiếu hình ảnh chuyên gia");
      return
    }
  ...
  async updateProduct() {
    ...
    const res = await axios({
      baseURL: Constants.BASE_URL,
      url: Constants.UPDATE_chuyengia_ITEM + this.state.id,
      method: 'PUT',
      data: body
    });
        ...
        <div className="animated fadeIn">
          <Row>
            <Col>
              <Card>
                <CardHeader>
                  <i className="fa fa-align-justify">Danh sách chuyên gia</i>
                      ...
                      <table ble className="table table-hover table-outline mb-0  d-sm-table">
                        <thead className="thead-light">
                          <tr>
                            ...
                            <th className="text-center">Tên chuyên gia</th>
                            <th className="text-center">Ảnh</th>
                            <th className="text-center">Mô tả</th> 
                            <th className="text-center">Link zalo</th>
                              ...
                              data.map((item, i) => {
                                return (
                                  <tr key={i}>
                                    ...
                                    <td className="text-center">
                                      <a target="_blank" href={item.linkdetail}>Link tư vấn</a>
                                    </td>
          ...
          <Modal size="xl" isOpen={this.state.modalCom} className={this.props.className}>
            ...
            <ModalBody>
              ...
              />
              {
                this.state.image == "" ? "" :
                  <img width="250" height="300" src={
                    this.state.image_show == "" ? `https://soida-api.placentor.com.vn/public/image_plugin/${this.state.image_link}` : this.state.image} style={{ marginBottom: 20 }} />
              }
              ...
              <TextFieldGroup
                field="linkdetail"
                label="Đường dẫn link zalo tư vấn"
                value={this.state.linkdetail}
                placeholder={"Đường dẫn link zalo tư vấn "}
